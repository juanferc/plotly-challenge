...
function render_chart(sample) {
    ...
    d3.json(file_path).then((data) => {
        ...
        var y_values = otu_ids.slice(0, 10).reverse().map(values => `OTU ${values}`)
        var hover_text = otu_labels.slice(0, 10).reverse();
        // Creating bar trace with its respective layout
        var bar_chart = {
            x: x_values,
            y: y_values,
            text: hover_text,
        ...
        // Create bubble trace with its respective layout
        var bubble_chart = {
            x: otu_ids,
            y: sample_values,
            text: otu_labels,
...
function panel_data(sample) {
    d3.json(file_path).then((data) => {
        ...
        // Create variables to hold values and places
